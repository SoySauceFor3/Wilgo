...
- In-place `update()` for started activities on content mismatch (kills the check-in blink) — only if dogfood shows the end+re-request blip is annoying; requires first verifying `update()` behavior on pending scheduled activities.
