...
// MARK: - HabitScheduling.psychDay(for:)
//
// Algorithm recap:
...
@Suite("HabitScheduling.psychDay(for:)")
struct HabitSchedulingPsychDayTests {
    ...
    struct ZeroOffsetTests {
        ...
        func morningMapsToday() {
            ...
            let result = HabitScheduling.psychDay(for: now, timeZoneIdentifier: "UTC", dayStartHourOffset: 0)
    ...
    struct Offset14Tests {
        ...
        func afterOffsetIsToday() {
            ...
            let result = HabitScheduling.psychDay(for: now, timeZoneIdentifier: "UTC", dayStartHourOffset: 14)
            #expect(result == utcDate(year: 2026, month: 1, day: 1))
        }
        ...
        func boundaryYieldsAdjacentPsychDays() {
            ...
            let dayBefore = HabitScheduling.psychDay(for: before, timeZoneIdentifier: "UTC", dayStartHourOffset: 14)
            let dayAfter  = HabitScheduling.psychDay(for: after,  timeZoneIdentifier: "UTC", dayStartHourOffset: 14)

            #expect(dayBefore < dayAfter)
            // Exactly one calendar day apart
            let diff = Calendar(identifier: .gregorian).dateComponents([.day], from: dayBefore, to: dayAfter).day!
            #expect(diff == 1)
        }
    }
    ...
    struct Offset2Tests {
        ...
        func threeAmIsToday() {
            ...
            let result = HabitScheduling.psychDay(for: now, timeZoneIdentifier: "UTC", dayStartHourOffset: 2)
            #expect(result == utcDate(year: 2026, month: 1, day: 1))
        }
    ...
    struct TimezoneTests {
        ...
        func utcVsNewYork_offset0() {
            ...
            let utcDay = HabitScheduling.psychDay(for: instant, timeZoneIdentifier: "UTC",              dayStartHourOffset: 0)
            let nyDay  = HabitScheduling.psychDay(for: instant, timeZoneIdentifier: "America/New_York", dayStartHourOffset: 0)
        ...
        @Test("year boundary: Jan 1 00:30 UTC with offset 2 shifts into Dec 31, crossing year boundary")
