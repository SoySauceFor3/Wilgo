...
///
/// Returns the ModelContainer alongside the context. Callers must keep the container
/// alive (via a local `let` binding) for the full test: ModelContext holds only a
/// weak back-reference to its container in SwiftData, so releasing it crashes.
@MainActor
...
final class CreditsUsedTests {
    ...
    @MainActor func noCheckIns() throws {
        let (ctx, _container) = try makeContext()
        let habit = Habit(
            title: "Run", createdAt: anchor, slots: [dummySlot()], skipCreditCount: 10,
    ...
    @MainActor func completedDayNotCounted() throws {
        ...
        ctx.insert(
            HabitCheckIn(habit: habit, createdAt: date(year: 2026, month: 3, day: 1, hour: 10)))
    ...
    @MainActor func allDaysCompleted() throws {
        ...
        for day in 1...4 {
            ctx.insert(
                HabitCheckIn(
                    habit: habit, createdAt: date(year: 2026, month: 3, day: day, hour: 10)))
        }
...
final class CreditsRemainingTests {
    ...
    @MainActor func simpleSubtraction() throws {
        ...
        let habit = Habit(
            title: "Run", createdAt: anchor, slots: [dummySlot()], skipCreditCount: 5,
            skipCreditPeriod: .monthly)
        ctx.insert(habit)
    ...
    @MainActor func flooredAtZero() throws {
        ...
        let habit = Habit(
            title: "Run", createdAt: anchor, slots: [dummySlot()], skipCreditCount: 2,
            skipCreditPeriod: .monthly)
        ctx.insert(habit)
    ...
    @MainActor func fullAllowanceWhenNoMisses() throws {
        let (ctx, _container) = try makeContext()
        let habit = Habit(
            title: "Run", createdAt: anchor, slots: [dummySlot()], skipCreditCount: 3,
            skipCreditPeriod: .monthly)
        ctx.insert(habit)
...
final class IsInPunishmentTests {
...
final class AnchorPeriodStartTests {
    ...
    func dailyAlwaysMidnight() {
        let now = date(year: 2026, month: 3, day: 5, hour: 8)
        let habit = Habit(
            title: "x", createdAt: date(year: 2026, month: 1, day: 17), slots: [],
            skipCreditCount: 0, skipCreditPeriod: .daily)
    ...
    func weeklyAnchorDayIsToday() {
        ...
        let habit = Habit(
            title: "x", createdAt: thursday, slots: [], skipCreditCount: 0,
    ...
    func weeklyAnchorEarlierInWeek() {
        ...
        let thursday = date(year: 2026, month: 3, day: 5)
        let habit = Habit(
            title: "x", createdAt: monday, slots: [], skipCreditCount: 0, skipCreditPeriod: .weekly)
        let start = SkipCreditService.periodStart(for: habit, now: thursday)
        #expect(start == monday)
    }
    ...
    func weeklySpanIsSevenDays() {
        let now = date(year: 2026, month: 3, day: 5, hour: 8)
        let habit = Habit(
            title: "x", createdAt: date(year: 2026, month: 3, day: 2), slots: [],
            skipCreditCount: 0, skipCreditPeriod: .weekly)
        let start = SkipCreditService.periodStart(for: habit, now: now)
    ...
    func monthlyAnchorDayIsToday() {
        ...
        let habit = Habit(
            title: "x", createdAt: march5, slots: [], skipCreditCount: 0, skipCreditPeriod: .monthly
        )
    ...
    func monthlyAnchorDayEarlierInMonth() {
        ...
        let habit = Habit(
            title: "x", createdAt: march4, slots: [], skipCreditCount: 0, skipCreditPeriod: .monthly
        )
    ...
    func monthlyAnchorDayLaterInMonth() {
        ...
        let habit = Habit(
            title: "x", createdAt: march20, slots: [], skipCreditCount: 0,
            skipCreditPeriod: .monthly)
        let start = SkipCreditService.periodStart(for: habit, now: march5)
        #expect(start == feb20)
    }
    ...
    func monthlyAnchor31InFebruary() {
        ...
        let habit = Habit(
            title: "x", createdAt: march31, slots: [], skipCreditCount: 0,
            skipCreditPeriod: .monthly)
    ...
    @MainActor func periodResetOnEdit() throws {
        ...
        let now = date(year: 2026, month: 3, day: 5, hour: 8)
        let habit = Habit(
            title: "Run", createdAt: createdAt, slots: [dummySlot()], skipCreditCount: 5,
            skipCreditPeriod: .monthly)
        ctx.insert(habit)
        #expect(SkipCreditService.creditsUsed(for: habit, now: now) == 4)
