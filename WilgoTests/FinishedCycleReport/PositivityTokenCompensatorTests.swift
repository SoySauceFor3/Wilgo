...
struct PositivityTokenCompensatorTests {
    ...
    func allocatesFcfs() {
        ...
        #expect(aided["older-cycle"]?.count == 1)
    ...
    func monthBoundaryUsesPreviousMonth() {
        ...
        let alreadyUsed = PositivityToken(
            reason: "used", createdAt: date(year: 2026, month: 1, day: 1))
        alreadyUsed.status = .used
        alreadyUsed.dayOfStatus = date(year: 2026, month: 2, day: 5)
        let active = PositivityToken(
            reason: "active", createdAt: date(year: 2026, month: 2, day: 6))
    ...
    @Test("consumed reasons are returned in FIFO order (oldest token first)")
    func compensatorReturnedReasonsInFIFOOrder() {
        let cycleEnd = date(year: 2026, month: 3, day: 10)
        let needs = [
            PositivityCycleNeed(
                cycleID: "cycle-fifo",
        ...
        let t1 = PositivityToken(reason: "older-reason", createdAt: date(year: 2026, month: 2, day: 1))
        let t2 = PositivityToken(reason: "newer-reason", createdAt: date(year: 2026, month: 2, day: 2))
        ...
        let reasons = aided["cycle-fifo"]
        #expect(reasons?.count == 2)
        #expect(reasons?[0] == "older-reason")
        #expect(reasons?[1] == "newer-reason")
    }

    @Test("consumed reasons are empty for unaided cycle")
    func consumedReasonsEmptyForUnaided() {
        let cycleEnd = date(year: 2026, month: 3, day: 10)
        let needs = [
            PositivityCycleNeed(
                cycleID: "cycle-full",
                commitmentID: UUID(),
                cycleEndPsychDay: cycleEnd,
                missingCheckIns: 0  // no missing check-ins
            )
        ]
        let t1 = PositivityToken(reason: "unused", createdAt: date(year: 2026, month: 2, day: 1))
        ...
        // Cycle with no missing check-ins should not appear in the result
        #expect(aided["cycle-full"] == nil)
        // Token should remain active
        if case .active = t1.status {
            // expected
        } else {
            Issue.record("t1 should remain active when no check-ins are missing")
        }
    }
}
